...
import { useGetViolationItemQuery, useGetViolationCountriesQuery } from '../../ViolationApi';
...
function ViolationView() {
	const { data: countries } = useGetViolationCountriesQuery({});
	...
	const {
		...
		isLoading,
		isError
	} = useGetViolationItemQuery(violationId, {
		skip: !violationId
	});
		...
		<>
			...
			<div className="relative flex flex-col flex-auto items-center p-6 pt-0 sm:p-12 sm:pt-0">
				<div className="w-full max-w-5xl">
					...
					<Typography className="mt-3 text-4xl font-bold truncate">{ violation.first_name}</Typography>

					<div className="flex flex-wrap items-center mt-2">
						{ violation?.tags?.map((id) => (
							<Chip
								key={id}
