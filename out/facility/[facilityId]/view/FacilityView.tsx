...
import { useGetFacilityItemQuery, useGetFacilityCountriesQuery } from '../../FacilityApi';
...
function FacilityView() {
	const { data: countries } = useGetFacilityCountriesQuery({});
	...
	const {
		...
		isLoading,
		isError
	} = useGetFacilityItemQuery(facilityId, {
		skip: !facilityId
	});
		...
		<>
			...
			<div className="relative flex flex-col flex-auto items-center p-6 pt-0 sm:p-12 sm:pt-0">
				<div className="w-full max-w-5xl">
					...
					<Typography className="mt-3 text-4xl font-bold truncate">{ facility.first_name}</Typography>

					<div className="flex flex-wrap items-center mt-2">
						{ facility?.tags?.map((id) => (
							<Chip
								key={id}
