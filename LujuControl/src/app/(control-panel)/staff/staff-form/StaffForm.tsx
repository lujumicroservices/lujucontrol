...
const schema = z.object({
	...
	role: z.string().min(1).max(100), // Varchar(100), not nullable
...
function StaffForm(props: StaffFormProps) {
	...
	const { data: staff, isError } = useGetStaffItemQuery(staffId, {
		skip: !staffId || staffId === null
	});

	const [createStaff] = useCreateStaffItemMutation();
	const [updateStaff] = useUpdateStaffItemMutation();
	const [deleteStaff] = useDeleteStaffItemMutation();
	...
	const onSubmit = useCallback(() => {
		if (isNew) {
			createStaff({ staff: form })
				.unwrap()
				.then((action) => {
					navigate(`/staff/${action.id}`);
				});
		} else {
			updateStaff({ id: staff.id, ...form });
		}
		// eslint-disable-next-line
	}, [form]);
